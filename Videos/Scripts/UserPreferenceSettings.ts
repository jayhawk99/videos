...
module UserPreferenceSettings {
	...
	export class Preferences {
		...
		configureDropdownListForSettingValue = function (selector, userpreferenceid, observableVar) {
        ...
        configureMultiForSettingValue = function (selector, userpreferenceid, arr) {
        ...
        configureRadiobuttonListForSettingValue = function (selector, userpreferenceid, observableVar) {
			$(selector).click(function () {
        ...
        configureCheckboxForSettingValue = function (selector, userpreferenceid, arr) {
			$(selector).click(function () {
				var values = [];
				for (var i = 0; i < arr().length; i++) {
					if (parseInt(arr()[i], 10) > 0) {
